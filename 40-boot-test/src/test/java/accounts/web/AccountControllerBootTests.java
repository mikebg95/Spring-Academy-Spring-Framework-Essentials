...
//		 - Verify that the response "Location" header contains "http://localhost/accounts/21"
		...
		.andExpect(header().string("Location", containsString("http://localhost/accounts/21")));
	...
	public void getAllAccounts() throws Exception {
		...
		for (int i = 0; i < 21; i++) {
			Account account = new Account("NUM_" + i, "ACC_" + i);
			mockAccounts.add(account);
		}

		given(accountManager.getAllAccounts()).willReturn(mockAccounts);
		...
		mockMvc.perform(get("/accounts")
				...
				.andExpect(jsonPath("$[0].name").value("ACC_0"))
				.andExpect(jsonPath("$[0].number").value("NUM_0"));
